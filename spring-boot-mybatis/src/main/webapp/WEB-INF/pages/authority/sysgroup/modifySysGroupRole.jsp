...
<%@ page import="com.sinosoft.normal.po.SysGroupRole" %>
...
<body>
	...
	<form id="fm" method="post">
		<input type="hidden" name="groupCode" id="groupCode" value="${queryUpdate.groupCode}">
	<div id="divTree">
				<script>
				var d = new dTree('d');
				...
				<%
					...
					List sysRoleList = (List)request.getAttribute("sysRolelist");
					List sysGroupRolelist = (List)request.getAttribute("sysGroupRolelist");
					SysRole role = null;
					SysGroupRole groupRole = null;
					for(int i=0; i <= sysRoleList.size()-1; i++){
						role = (SysRole)sysRoleList.get(i);
						isCheck = false;
						for(int j=0; j <= sysGroupRolelist.size()-1; j++){
							groupRole = (SysGroupRole)sysGroupRolelist.get(j);
							if(groupRole.getRoleCode().equals(role.getRoleCode())){
								isCheck = true;
							}
						}
						if(isCheck){
							out.println("d.add('"+role.getRoleCode()+"','"+role.getUpperRoleCode()+"','"+role.getRoleName()+"','','','','','','',true,true,false);");
						}else{
...
<tr>
	...
	<td align="center" ><input class="button" type="button" id="btn" value="保存" ></td>
...
$("#btn").click(function () {
	...
	//alert(typeof treeCheckBox);
	//alert(groupCodeValue);
  var url="${ctx}/sysGroup/saveUpdateSysGroupRole.do";
